...
namespace PlmonFuncTestNunit.PageObjects
{
    public class PageObjectStyle : PageBase
    {
        ...
        public IWebElement spanView { get; set; }

        [FindsBy(How = How.Id, Using = "tb_1")]
        public IWebElement largeThumbnails  { get; set; }
        ...
        public void CheckView()
        {
            ...
            spanView.Click();
            //Large Thumbnails 
            largeThumbnails.Click();
            SeleniumGetMethod.WaitForPageLoad(driver);
            spanView.Click();
            SeleniumGetMethod.WaitForPageLoad(driver);

            PropertiesCollection._reportingTasks.Log(Status.Info, "Check change color background ");
            PropertiesCollection._reportingTasks.LogAddScreenElement(driver, "tb_1", "spanView",SelectType.Id);
            string afterColor = largeThumbnails.GetCssValue("background-color");


            Assert.IsTrue(afterColor.Contains("rgba(93, 98, 100, 1)"));
            Assert.NotNull(listlargeThumbnails);
            var countLarge = listlargeThumbnails.Count;
            PropertiesCollection._reportingTasks.Log(Status.Info, "Count thumb = "+ countLarge);
            //Medium Thumbnails 
            SeleniumGetMethod.WaitForPageLoad(driver);
  

            mediumThumbnails.Click();
            SeleniumGetMethod.WaitForPageLoad(driver);
            spanView.Click();
            SeleniumGetMethod.WaitForPageLoad(driver);

            PropertiesCollection._reportingTasks.Log(Status.Info, "Check change color background ");
            PropertiesCollection._reportingTasks.LogAddScreenElement(driver, "tb_2", "mediumThumbnails", SelectType.Id);
            afterColor = largeThumbnails.GetCssValue("background-color");

            Assert.IsTrue(afterColor.Contains("rgba(93, 98, 100, 1)"));
            Assert.NotNull(listlargeThumbnails);
            var countMedium = listlargeThumbnails.Count;
            PropertiesCollection._reportingTasks.Log(Status.Info, "Count thumb = " + countMedium);
            //Assert.AreNotEqual(countMedium, countLarge);
