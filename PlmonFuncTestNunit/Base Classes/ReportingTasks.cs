...
namespace PlmonFuncTestNunit.Base_Classes
{
    public class ReportingTasks: GetSreenShot
    {
        ...
        public void LogAddScreenElement(IWebDriver driver, string elementVal, string name, SelectType type)
        {
            
            var img = GetElementScreenShort(driver, elementVal, name, type);
            _test.Log(Status.Debug, name + _test.AddScreenCaptureFromPath(img));
        }
